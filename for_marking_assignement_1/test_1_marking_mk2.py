...
class TestApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def do_post(url_to_shorten):
            ...
            response = requests.post(url, json={'long_url': str(url_to_shorten)})
    ...
    def test_get_request_with_id_expect_301(self):
        ...
        self.assertEqual(response.json().get("long_url"), expected_value, "Expected response body to be " +expected_value+" , but got "+response.json().get("long_url"))
    ...
    def test_put_id(self):
        ...
        endpoint = "/"
        url = f"{self.base_url}{endpoint}{id}"   
        response = requests.put(url, json ={'long_url': str(url_after_update)})
        self.assertEqual(response.status_code, 200, f"Expected status code 200, but got {response.status_code}")

        #check value has been rally changed by doing a get
        url = f"{self.base_url}{endpoint}{id}"
        response = requests.get(url)
        self.assertEqual(response.json().get("long_url"), url_after_update, "Expected response body to be " +url_after_update+" , but got "+response.json().get("long_url"))

        
        #check 400 by passing invalid url
        url = f"{self.base_url}{endpoint}{id}"   
        response = requests.put(url, json= {'long_url': invalid_url})
        ...
        response = requests.put(url, json= {'long_url': not_existing_id})
        self.assertEqual(response.status_code, 404, f"Expected status code 404, but got {response.status_code}")
    ...
    def test_post(self):
        ...
        response = requests.get(url)
        self.assertEqual(response.status_code, 301, f"Expected status code 301, but got {response.status_code}")
        self.assertEqual(response.json().get("long_url"), url_to_shorten, "Expected response body to be " +url_to_shorten+" , but got "+response.json().get("long_url"))
